...
String apiKey =
    "0D6B765B66C94A75B89233E93946C0EA"; //api key that allows for application to control server without user authentication
...
Future<http.Response> selectPrint(String filename) async {
  ...
  var send = json.encode({"command": "select", "print": true});
...
Future<http.Response> startDemo() async {
  String hostName =
      "http://192.168.23.241/api/files/local/PolarBearCapstone.gcode";
...
Future<http.Response> pauseJob() async {
  ...
  var body = json.encode({"command": "pause", "action": "pause"});
...
Future<http.Response> resumeJob() async {
  ...
  var body = json.encode({"command": "pause", "action": "resume"});
